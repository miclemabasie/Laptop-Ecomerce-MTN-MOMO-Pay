...
class PayClass:
    ...
    collections_subkey = settings.COLLECTION_PRIVATE_KEY

    # Disbursement subscription key
    disbursements_subkey = settings.DISBURSMENT_PRIMARY_KEY

    # Production collections basic authorisation key(Leave it blank if in sandbox mode)
    basic_authorisation_collections = settings.COLLECTION_SECONDARY_KEY

    # Production disbursement basic authorisation key(Leave it blank if in sandbox mode)
    basic_authorisation_disbursments = settings.DISBURSMENT_SECONDARY_KEY
    ...
    def verifymomo(txn):
        url = (
            "" + str(PayClass.accurl) + "/collection/v1_0/requesttopay/" + str(txn) + ""
        )
    ...
    def momobalancedisbursement():
        ...
        headers = {
            ...
            + str(PayClass.momotokendisbursement()["access_token"]),
            "X-Target-Environment": PayClass.environment_mode,
        }
